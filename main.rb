class Game
  ...
  def display_table
    @play_table.each do |line|
      puts line.join('|')
    end
  end

  def play_round(player)
    return finish_game(nil) if @choices_number == 9
  ...
  def did_win?(_player)
    ...
    while i < 3
      j = 0
      return true if @play_table[i][j] == @play_table[i][j + 1] && @play_table[i][j] == @play_table[i][j + 2]
      return true if @play_table[j][i] == @play_table[j + 1][i] && @play_table[j][i] == @play_table[j + 2][i]

      i += 1
    end
    return true if @play_table[0][0] == @play_table[1][1] && @play_table[0][0] == @play_table[2][2]
    return true if @play_table[2][0] == @play_table[1][1] && @play_table[1][1] == @play_table[0][2]
